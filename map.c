...
int recur(int i) {      /* a recursive function */
    ...
    printf("recur call %d: stack@ %lx\n", i, (long)&j); /* fix this so it prints the address of j */
    if (i > 0) {
        return recur(i-1);
    }
    return 0;
...
int main (int argc, char *argv[]) {
    ...
    char *buf1 = (char*)malloc(100); /* dynamically allocate some stuff */
    char *buf2 = (char*)malloc(100); /* and some more stuff */
    printf("_main  @ %lx\n", (long unsigned int)&main); /* fix to print address of main */
    printf("recur @ %lx\n", (long unsigned int)&recur); /* fix to print address of recur */
    printf("_main stack: %lx\n", (long)&i); /* fix to get address of the stack variable */
    printf("static data: %lx\n", (long)&stuff); /* fix to get address of a static variable */
